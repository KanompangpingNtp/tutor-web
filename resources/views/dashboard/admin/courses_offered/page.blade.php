                ...
                <div class="modal fade" id="editCourseModal{{ $course->id }}" tabindex="-1" aria-labelledby="editCourseModalLabel{{ $course->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <form action="{{ route('updateCourse', $course->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title">แก้ไขข้อมูลคอร์ส</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="ปิด"></button>
                                </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        <div class="form-floating">
                                            <textarea class="form-control" placeholder="รายละเอียดคอร์ส" id="details_update" name="course_details">{{ $course->course_details }}</textarea>
                                        </div>
                                    </div>

                                    {{-- 🔹 อัปโหลดใหม่ --}}
                                    <div class="mb-3">
                                        <label class="form-label">อัปโหลดปกคอร์ส (ไฟล์ภาพ)</label>
                                    ...
                                    {{-- 🔹 แสดงไฟล์ที่อัปโหลดไว้ --}}
                                    @if($course->files && $course->files->count())
                                        <div class="mb-3">
                                            <label class="form-label">ไฟล์ที่เคยอัปโหลด</label>
                                            <ul class="list-group">
                                                @foreach($course->files as $file)
                                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                                        ...
                                                        <form action="{{ route('deleteFile', $file->id) }}" method="POST" onsubmit="return confirm('ยืนยันการลบไฟล์นี้?');">
                                                            @csrf
                                                            @method('DELETE')

                ...
                <div class="row">
                    ...
                    <div class="modal fade" id="userModal{{$users->id}}" tabindex="-1" aria-labelledby="userModalLabel{{$users->id}}" aria-hidden="true">
                        <div class="modal-dialog">
                            <form action="{{ route('TeacherInformationUpdate', $users->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                    ...
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{ route('TutorTeacherResumeCreate') }}" method="POST" class="container mt-2">
                                    @csrf
                                    <div class="mb-3">
...
{{-- <script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        addBtn.addEventListener("click", function() {
            const currentSets = group.querySelectorAll(".education-set").length;

            if (currentSets < maxEducation) {
                const clone = group.querySelector(".education-set").cloneNode(true);
                // ล้างค่าชุดใหม่
                clone.querySelectorAll("input").forEach(input => input.value = "");
